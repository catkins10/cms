...
import com.yuanluesoft.credit.enterprisecredit.market.forms.ImportPersonDateExcel;
 ...
 *
 */
public class MarketPersonImportServiceImpl extends BusinessServiceImpl {
	...
	public void importData(ImportPersonDateExcel excel, SessionInfo sessionInfo)
	...
	private void importDate(Attachment attachment, SessionInfo sessionInfo)
		...
		int firstRow;
		int lastRow;
		String[] colName = new String[] { "序号", "注册号", "字号名称","经营者",
				"经营场所", "成立日期" ,"登记机关","工商所","片区","信用类别","信用分值","电话","经营范围","经营期限起","经营期限止"};
		try {
		...
		for (int contentRow = firstRow + 1; contentRow <= lastRow; contentRow++) {
			...
			MarketPerson marketPerson = new MarketPerson();
			marketPerson.setCreator(sessionInfo.getUserName());
			marketPerson.setCreatorId(sessionInfo.getUserId());
			marketPerson.setCreated(DateTimeUtils.now());
			marketPerson.setId(UUIDLongGenerator.generateId());
			
			marketPerson.setRegistCode(getStringValue(content.getCell(colNum[1]))) ; //注册号
			marketPerson.setName(getStringValue(content.getCell(colNum[2]))); //字号名称
			marketPerson.setPerson(getStringValue(content.getCell(colNum[3])));//经营者
			marketPerson.setAddr(getStringValue(content.getCell(colNum[4])));//经营场所
			marketPerson.setBuildDate(getDateValue(content.getCell(colNum[5]),"yyyy-MM-dd"));//成立日期
			marketPerson.setRegistrar(getStringValue(content.getCell(colNum[6])));//登记机关
			marketPerson.setAic(getStringValue(content.getCell(colNum[7])));//工商所
			marketPerson.setArea(getStringValue(content.getCell(colNum[8])));//片区
			marketPerson.setCreditType(getStringValue(content.getCell(colNum[9])));//信用类别
			marketPerson.setCreditScore(getDoubleValue(content.getCell(colNum[10])));//信用分值
			marketPerson.setTel(getStringValue(content.getCell(colNum[11])));//电话
			marketPerson.setBusinessScope(getStringValue(content.getCell(colNum[12])));//经营范围
			marketPerson.setStartDate(getDateValue(content.getCell(colNum[13]),"yyyy-MM-dd"));//经营期限起
			marketPerson.setEndDate(getDateValue(content.getCell(colNum[14]),"yyyy-MM-dd"));//经营期限止
			save(marketPerson);
