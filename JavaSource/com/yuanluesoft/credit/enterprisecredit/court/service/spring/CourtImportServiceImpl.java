package com.yuanluesoft.credit.enterprisecredit.court.service.spring;
...
import com.yuanluesoft.credit.enterprisecredit.court.forms.ImportDateExcel;
 ...
 *
 */
public class CourtImportServiceImpl extends BusinessServiceImpl {
	...
	public void importData(ImportDateExcel excel, SessionInfo sessionInfo)
		...
		List attachments = attachmentService.list(
				"credit/enterprisecredit/court", "data", excel.getId(),false, 1,
				null);
		...
		try {
			...
			attachmentService.deleteAll("credit/enterprisecredit/court",
	...
	private void importDate(Attachment attachment, SessionInfo sessionInfo)
		...
		int firstRow;
		int lastRow;
		String[] colName = new String[] { "序号", "姓名", "类型",
				"执行案号", "执行法院" ,"发布时间" ,"最后更新时间","状态"};
		try {
		...
		for (int contentRow = firstRow + 1; contentRow <= lastRow; contentRow++) {
			...
			court.setCreator(sessionInfo.getUserName());
			court.setCreatorId(sessionInfo.getUserId());
			court.setCreated(DateTimeUtils.now());
			court.setId(UUIDLongGenerator.generateId());
			court.setName(getStringValue(content.getCell(colNum[1]))) ; //姓名／名称
			court.setType(getStringValue(content.getCell(colNum[2]))); //类型
			court.setCode(getStringValue(content.getCell(colNum[3])));//执行案号
			court.setCourt(getStringValue(content.getCell(colNum[4])));//执行法院
			court.setIssueTime(getDateValue(content.getCell(colNum[5]),"yyyy-MM-dd HH:mm:ss"));//发布时间
			court.setUpdateTime(getDateValue(content.getCell(colNum[6]),"yyyy-MM-dd HH:mm:ss"));//最后更新时间
			court.setStatus(getStringValue(content.getCell(colNum[7])));//状态
			save(court);
