 ...
 *
 */
public class EnterpriseImportServiceImpl extends BusinessServiceImpl {
	...
	private void importDate(Attachment attachment, SessionInfo sessionInfo)
		...
		int firstRow;
		int lastRow;
		String[] colName = new String[] { "注册号", "名称", "组成形式",
				"资金数额(万元)", "经营场所" ,"行业门类" ,"行业类别","行业代码","经营范围","经营者","联系电话","开业日期","核准日期","属地工商所","片区"};
		try {
		...
		for (int contentRow = firstRow + 1; contentRow <= lastRow; contentRow++) {
			...
			String loginName = CnToSpell.getShortSpell(getStringValue(content.getCell(colNum[1])));
			enterprise.setPassword("abc,123");//密码
			enterprise.setRegistCode(getStringValue(content.getCell(colNum[0])));//注册号
			enterprise.setName(getStringValue(content.getCell(colNum[1])));//名称
			enterprise.setComposition(getStringValue(content.getCell(colNum[2])));//组成形式
			enterprise.setWorth(getDoubleValue(content.getCell(colNum[3])));//资金数额(万元)
			enterprise.setAddr(getStringValue(content.getCell(colNum[4])));//经营场所
			enterprise.setDoorType(getStringValue(content.getCell(colNum[5])));//行业门类
			enterprise.setIndustry(getStringValue(content.getCell(colNum[6])));//行业类别
			enterprise.setCode(getStringValue(content.getCell(colNum[7])));//行业代码
			enterprise.setBusinessScope(getStringValue(content.getCell(colNum[8])));//经营范围
			enterprise.setPerson(getStringValue(content.getCell(colNum[9])));//经营者
			enterprise.setTel(getStringValue(content.getCell(colNum[10])));//联系电话
			enterprise.setOpenDate(getDateValue(content.getCell(colNum[11]), "yyyy-MM-dd"));//开业日期
			enterprise.setApprovalDate(getDateValue(content.getCell(colNum[12]), "yyyy-MM-dd"));//核准日期
			enterprise.setAscription(getStringValue(content.getCell(colNum[13])));//属地工商所
			enterprise.setArea(getStringValue(content.getCell(colNum[14])));//片区
			String hql = "from Enterprise Enterprise where Enterprise.registCode = '"+enterprise.getRegistCode()+"'";
			Enterprise old = (Enterprise)getDatabaseService().findRecordByHql(hql);
			if(old!=null){
				old.setName(getStringValue(content.getCell(colNum[1])));//名称
				old.setComposition(getStringValue(content.getCell(colNum[2])));//组成形式
				old.setWorth(getDoubleValue(content.getCell(colNum[3])));//资金数额(万元)
				old.setAddr(getStringValue(content.getCell(colNum[4])));//经营场所
				old.setDoorType(getStringValue(content.getCell(colNum[5])));//行业门类
				old.setIndustry(getStringValue(content.getCell(colNum[6])));//行业类别
				old.setCode(getStringValue(content.getCell(colNum[7])));//行业代码
				old.setBusinessScope(getStringValue(content.getCell(colNum[8])));//经营范围
				old.setPerson(getStringValue(content.getCell(colNum[9])));//经营者
				old.setTel(getStringValue(content.getCell(colNum[10])));//联系电话
				old.setOpenDate(getDateValue(content.getCell(colNum[11]), "yyyy-MM-dd"));//开业日期
				old.setApprovalDate(getDateValue(content.getCell(colNum[12]), "yyyy-MM-dd"));//核准日期
				old.setAscription(getStringValue(content.getCell(colNum[13])));//属地工商所
				old.setArea(getStringValue(content.getCell(colNum[14])));//片区
				update(old);
