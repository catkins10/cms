 ...
 *
 */
public class EnterpriseInImportServiceImpl extends BusinessServiceImpl {
	...
	private void importDate(Attachment attachment, SessionInfo sessionInfo)
		...
		int firstRow;
		int lastRow;
		String[] colName = new String[] { "注册号", "统一社会信用代码", "登记名称",
				"企业类别", "登记类型" ,"企业类型" ,"法定代表人","注册资本(万元)","联系电话","公示联络员","联络员电话","行业门类","行业类别","行业代码","经营范围","成立日期","营业期限","核准日期","住所","管片工商所","片区"};
		try {
		...
		for (int contentRow = firstRow + 1; contentRow <= lastRow; contentRow++) {
			...
			EnterpriseIn enterprise = new EnterpriseIn();
			...
			enterprise.setName(getStringValue(content.getCell(colNum[2])));//登记名称
			enterprise.setCategory(getStringValue(content.getCell(colNum[3])));//企业类别
			enterprise.setRegistType(getStringValue(content.getCell(colNum[4])));//登记类型

			enterprise.setType(getStringValue(content.getCell(colNum[5])));//企业类型
			enterprise.setPerson(getStringValue(content.getCell(colNum[6])));//法定代表人\负责人
			enterprise.setWorth(getDoubleValue(content.getCell(colNum[7])));//注册资本(万元)
			enterprise.setTel(getStringValue(content.getCell(colNum[8])));//联系电话
			enterprise.setLinkMan(getStringValue(content.getCell(colNum[9])));//公示联络员

			enterprise.setLinkTel(getStringValue(content.getCell(colNum[10])));//联络员电话
			enterprise.setDoorType(getStringValue(content.getCell(colNum[11])));//行业门类
			enterprise.setIndustry(getStringValue(content.getCell(colNum[12])));//行业类别
			enterprise.setCode(getStringValue(content.getCell(colNum[13])));//行业代码
			enterprise.setBusinessScope(getStringValue(content.getCell(colNum[14])));//经营范围

			enterprise.setStartDate(getDateValue(content.getCell(colNum[15]),"yyyy-MM-dd"));//成立日期
			enterprise.setLimitDate(getStringValue(content.getCell(colNum[16])));//营业期限
			enterprise.setApprovalDate(getDateValue(content.getCell(colNum[17]),"yyyy-MM-dd"));//核准日期
			enterprise.setAddr(getStringValue(content.getCell(colNum[18])));//住所
			enterprise.setAscription(getStringValue(content.getCell(colNum[19])));//管片工商所
			enterprise.setArea(getStringValue(content.getCell(colNum[20])));//片区
			String hql = "from EnterpriseIn EnterpriseIn where EnterpriseIn.registCode = '"+enterprise.getRegistCode()+"'";
			EnterpriseIn old = (EnterpriseIn)getDatabaseService().findRecordByHql(hql);
			if(old!=null){
				old.setCreditCode(getStringValue(content.getCell(colNum[1])));//统一社会信用代码
				old.setName(getStringValue(content.getCell(colNum[2])));//登记名称
				old.setCategory(getStringValue(content.getCell(colNum[3])));//企业类别
				old.setRegistType(getStringValue(content.getCell(colNum[4])));//登记类型

				old.setType(getStringValue(content.getCell(colNum[5])));//企业类型
				old.setPerson(getStringValue(content.getCell(colNum[6])));//法定代表人\负责人
				old.setWorth(getDoubleValue(content.getCell(colNum[7])));//注册资本(万元)
				old.setTel(getStringValue(content.getCell(colNum[8])));//联系电话
				old.setLinkMan(getStringValue(content.getCell(colNum[9])));//公示联络员

				old.setLinkTel(getStringValue(content.getCell(colNum[10])));//联络员电话
				old.setDoorType(getStringValue(content.getCell(colNum[11])));//行业门类
				old.setIndustry(getStringValue(content.getCell(colNum[12])));//行业类别
				old.setCode(getStringValue(content.getCell(colNum[13])));//行业代码
				old.setBusinessScope(getStringValue(content.getCell(colNum[14])));//经营范围

				old.setStartDate(getDateValue(content.getCell(colNum[15]),"yyyy-MM-dd"));//成立日期
				old.setLimitDate(getStringValue(content.getCell(colNum[16])));//营业期限
				old.setApprovalDate(getDateValue(content.getCell(colNum[17]),"yyyy-MM-dd"));//核准日期
				old.setAddr(getStringValue(content.getCell(colNum[18])));//住所
				old.setAscription(getStringValue(content.getCell(colNum[19])));//管片工商所
				old.setArea(getStringValue(content.getCell(colNum[20])));//片区
				update(old);
