package com.yuanluesoft.jeaf.workflow.connectionpool.spring;
 ...
 *
 */
public class WorkflowConnectionPoolImpl implements WorkflowConnectionPool {
	...
	/*
	 * (non-Javadoc)
	 * @see com.yuanluesoft.jeaf.form.workflowform.service.WorkflowConnectionPool#getWorkflowClient(java.lang.String)
	 */
	public WorkflowClient getWorkflowClient(SessionInfo sessionInfo) throws ServiceException {
		synchronized(workflowConnections) {
			...
			if(workflowConnection.getCurrentUserId()!=sessionInfo.getUserId()) { //当前正在连接的用户不是需要的用户
				try {
					...
					connectInfo.setUserName(sessionInfo.getUserName());
					connectInfo.setUserId(sessionInfo.getUserId());
					connectInfo.setDeparmentId(sessionInfo.getDepartmentId());
					connectInfo.setDeparmentIds(sessionInfo.getDepartmentIds());
					connectInfo.setRoleIds(sessionInfo.getRoleIds());
