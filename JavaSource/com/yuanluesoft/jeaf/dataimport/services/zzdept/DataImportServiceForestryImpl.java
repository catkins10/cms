...
/**
 * 漳州部门网站(使用SqlServer的网站)
 * @author linchuan
 *
 */
public class DataImportServiceForestryImpl extends DataImportService {
		...
		dataImporters.add(new SiteDataImporter() {
			...
			public String[] generateRetrieveDataSQL(List selectedSourceDirectoryIds) {
				return new String[] {
					   ...
					   "(select min(doc_category_assoc.cat_id) from doc_category_assoc where doc_category_assoc.doc_id=doc.id) as directoryId," + //原来的栏目ID
					   "title as subject," + //标题
					   ...
					   "approved_date as issueTime," + //发布时间
					   "authors as Editor," + //创建者
			...
			new PublicInfoImporter() {
				...
				public String[] generateRetrieveDataSQL(List selectedSourceDirectoryIds) {
					return new String[] {"select " +
							   ...
							   "remark as mark," + //文号
							   "format(left(createdate,4) + '/' + mid(createdate,5,2) + '/' + right(createdate, 2),'yyyy/mm/dd') as generateDate," + //生成日期
							   "title as subject," + //标题
							   "content as body," + //正文
