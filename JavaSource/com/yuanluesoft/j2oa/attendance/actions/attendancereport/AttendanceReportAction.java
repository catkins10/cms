 ...
 *
 */
public class AttendanceReportAction extends DialogFormAction {
	...
	public void checkLoadPrivilege(ActionForm form, HttpServletRequest request, List acl, SessionInfo sessionInfo) throws PrivilegeException, SystemUnregistException {
		if(!acl.contains("application_manager") && !acl.contains("manageUnit_attendance")) {
	...
	public void initForm(ActionForm form, List acl, SessionInfo sessionInfo, HttpServletRequest request) throws Exception {
		...
		date = DateTimeUtils.add(date, Calendar.DAY_OF_MONTH, -1);
		report.setEndDate(date);
		date = DateTimeUtils.set(date, Calendar.DAY_OF_MONTH, 1);
		report.setBeginDate(date);
	}
}