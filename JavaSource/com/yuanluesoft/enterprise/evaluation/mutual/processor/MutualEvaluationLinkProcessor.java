package com.yuanluesoft.enterprise.evaluation.mutual.processor;
 ...
 *
 */
public class MutualEvaluationLinkProcessor  implements PageElementProcessor {
	...
	public void writePageElement(HTMLElement pageElement, WebDirectory webDirectory, WebSite parentSite, HTMLParser htmlParser, SitePage sitePage, RequestInfo requestInfo, HttpServletRequest request) throws ServiceException {
		...
		if(mutualEvaluationService.listToEvaluatePersons(RequestUtils.getSessionInfo(request))==null) {
			pageElement.getParentNode().removeChild(pageElement);
			return;
		}
		pageElement.removeAttribute("id");
		//输出链接
		LinkUtils.writeLink(a, "/enterprise/evaluation/mutual/evaluation.shtml", a.getTarget(), webDirectory.getId(), parentSite.getId(), null, false, true, sitePage, request);
	}
