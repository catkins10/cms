...
public class Download extends FetBaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			year = Integer.parseInt(importQueryForm.getQueryYear());
		}
		catch(Exception e) {

		}
		try {
			month = Integer.parseInt(importQueryForm.getQueryMonth());
		}
		catch(Exception e) {
		...
		tradeStatService.exportImportData(ListUtils.generateList(countyCode, ","), ListUtils.generateList(countyName, ","), ListUtils.generateList(developmentAreaCode, ","), null, companyCode, importQueryForm.getQueryCompany(), year, month, response);
		return null;
    }
}