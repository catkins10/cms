 ...
 *
 */
public class FdiCustomerViewServiceImpl extends ViewServiceImpl {
	...
	public void retrieveViewPackage(ViewPackage viewPackage, View view, int beginRow, boolean retrieveDataOnly, boolean readRecordsOnly, boolean countRecordsOnly, HttpServletRequest request, SessionInfo sessionInfo) throws ServiceException, PrivilegeException {
		...
		if(industryIds==null || industryIds.isEmpty()) { //对行业没有编辑权限
			ListUtils.removeObjectByProperty(view.getActions(), "title", "添加单位"); //删除“添加单位”操作
		}
		//检查用户对应用的权限
		...
		if(acl==null || !acl.contains(AccessControlService.ACL_APPLICATION_MANAGER)) { //不是管理员,管理员允许查看全部
			...
			else if(pojoClassName.equals("FdiCustomerCompany")) { //客户单位视图
				view.addWhere("industries.industryId in (" + JdbcUtils.validateInClauseNumbers(ListUtils.join(industryIds, ",", false)) + ")");
			}
			else if(pojoClassName.equals("FdiCustomerContact")) { //客户联系人视图
				view.addJoin(", FdiCustomerCompanyIndustry FdiCustomerCompanyIndustry");
				view.addWhere("FdiCustomerCompanyIndustry.companyId=FdiCustomerContact.companyId and FdiCustomerCompanyIndustry.industryId in (" + JdbcUtils.validateInClauseNumbers(ListUtils.join(industryIds, ",", false)) + ")");
			}
		}
