			...
			try {
				...
				if(materials!=null && !materials.trim().isEmpty()) {
					...
					for(int j=0; j<itemMaterials.length; j++) {
						...
						material.setName(itemMaterials[j]); //申报材料名称
						//material.setDescription(description); //申报说明
						material.setPriority(itemMaterials.length - j); //优先级
						//material.setTableName(tableName); //表格名称
						//material.setTableURL(tableURL); //表格URL
						//material.setExampleURL(exampleURL); //样表URL
						//material.setRemark(remark); //备注
						save(material);
					}
				}
			}
