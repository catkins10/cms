 ...
 *
 */
public class SceneServiceLinkProcessor implements PageElementProcessor { 
	...
	public void writePageElement(HTMLElement pageElement, WebDirectory webDirectory, WebSite parentSite, HTMLParser htmlParser, SitePage sitePage, RequestInfo requestInfo, HttpServletRequest request) throws ServiceException {
		...
		LinkUtils.writeLink(anchorElement, "/cms/scene/" + (!"true".equals(StringUtils.getPropertyValue(urn, "sceneTree")) ? "sceneService" : "sceneIndex") + ".shtml?id=" + StringUtils.getPropertyValue(urn, "id"), anchorElement.getTarget(), parentSite.getId(), parentSite.getId(), null, false, true, sitePage, request);
	}
