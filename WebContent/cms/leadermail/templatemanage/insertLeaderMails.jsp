...
<ext:form action="/templatemanage/insertLeaderMails">
	...
	<script type="text/javascript">
		...
		function getElementTitle(isSearchResults, isRssChannel) {
			if(isRssChannel) {
				...
				return "来信列表" + (document.getElementsByName("leaderMailTypes")[0].value=="" ? "" : ":" + document.getElementsByName("leaderMailTypes")[0].value);
			}
		}
		//显示扩展属性,由继承者实现
		function showExtendProperties(recordListProperties, recordListTextContent) {
			...
			document.getElementsByName("leaderMailTypes")[0].value = StringUtils.getPropertyValue(recordListProperties, "leaderMailTypes");
		...
		function getExtendPropertiesAsText() {
			var extendProperties = "leaderMailTypes=" + document.getElementsByName("leaderMailTypes")[0].value;
		...
		//选择信件类型
		function selectMailTypes() {
			DialogUtils.openSelectDialog("cms/leadermail", "selectLeaderMailType", 560, 360, true, "leaderMailTypes{type},leaderMailTypes{type|信件类型|100%}", "", "", "", ",", false, "siteId=" + editor.document.getElementsByName("siteId")[0].value);
		}
	...
	<table border="0" width="100%" cellspacing="5" cellpadding="0">
		...
		<tr valign="middle">
			...
			<td><ext:field property="leaderMailTypes"/></td>
		</tr>
	</table>
</ext:form>