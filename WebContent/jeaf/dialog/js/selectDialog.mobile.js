...
function initResults() {
	...
	// by zyh
			var selectedData = new Array();//divData中的数据行 tr
			if(frames["frameViewData"]){
				var doc = frames["frameViewData"].document;
				var selects = doc.getElementsByName("select");
				if(selects) {
...
//add by zyh  将divData数据填充到divResult中
function addSeleted() {
	var selectedData = new Array();
	if(frames["frameViewData"]){
		var doc = frames["frameViewData"].document;
		var selects = doc.getElementsByTagName("input");
		if(selects) {
			for(var i=0; i<selects.length; i++) {
				if(selects[i].checked) {
					selectedData[selectedData.length] = selects[i].parentElement.parentElement;//选中的数据行 tr节点
				}
			}
		}
	}else{
